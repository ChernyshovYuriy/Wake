...
| **F8-1** equity restriction re-implemented in `app/vet.py` and `backtest/signal_source.py` | `wallets/scoring/slice.equity_fills(fills, equities)` is the single home; `EquitySlice.from_history`, the vet prescreen and the backtest reconstruction call it. The remaining `in equities` elsewhere filters *markets* (a different registry row) | `test_slice.py::test_equity_fills_keeps_only_equity_symbols_in_order` |
| **F7-1** a curated prior score without `as_of` silently never applied in backtests | `run_backtest` adds a caveat when prior scores are dated after the first session's pre-open view (the look-ahead guard hides them until then), naming the curated-`as_of` cause | `test_backtest.py`: late scores reported; on-time scores and a session-less period produce no caveat |
| **B-3** tests themselves not measured | `[tool.coverage.run] source = ["hlsignals", "tests"]` (`tests/live` omitted); the 100% gate in `ci.sh` includes `tests/*`; tripwire `raise AssertionError` lines are excluded by pattern; snapshot-regeneration branches are `no cover`. Dead test code found and removed: an unused "replace bar" path in the replay price helper, an unreachable fake-websocket branch, and an unreachable `return` after a call that always raises. Real branches got tests instead: `MomentumSource` with too little history or a flat price, and the network guard's pass-through for local Unix sockets | The gate is live: with a test deselected, CI fails on that test's now-uncovered line |
| **Phase 5 minors** | `test_census.py`: exact `WalletObservation` (first/last seen, count) for both counterparties. `test_vet.py`: a partial first page is fully read (20 fills) and judged by the chain (`min_sample`), not the prescreen | — |
| **T-4 / Phase 4 correction** | Real-fixture tests in `test_lots.py`. (1) FIFO PnL vs the API `closedPnl` on every non-orphan round trip of the 4000-fill capture. **Correction to Phase 4:** I reported "exact on 3/3", but my script only flagged differences above 0.01. xyz:BB differs by 0.00048 (−7.97091 vs −7.97043). `LotBook` matches an independent exact average-cost recomputation to the last digit; the API's per-fill `closedPnl` drifts by up to 9e-5 per fill because the exchange uses a rounded average entry price. The test allows 1 ppm of the trip's entry notional (BB: 0.4 ppm). (2) Same-millisecond ordering: 100+ groups chain `startPosition` in API order, none in `tid` order, which pins why `lots.py` departs from plan §8 | `test_fifo_pnl_agrees_with_the_api_closed_pnl_on_every_real_round_trip`, `test_real_same_millisecond_fills_chain_in_api_order_not_tid_order` |
| **F1-1 / F1-2** `dir` values mapped without a fixture | A live scan followed `liquidation.liquidatedUser` pointers from census wallets (211 `userFills` calls). **All four liquidation values were observed** (`Liquidated Isolated Long` on xyz:BRENTOIL, `Isolated Short` on CASHCAT, `Cross Long` on ETH, `Cross Short` on BTC); each is a full close with the mapped sign and fee 0. The scan also found **two real values that had no mapping**: `Settlement` (IP, delisted market) and `Auto-Deleveraging` (CASHCAT). Their sign follows the position, so `direction.py` derives it from `startPosition` and rejects any fill that does not fully close (settlement) or strictly reduce (ADL) the position. The four non-perp values now use the real Phase 0 fills | `dir_catalog.json` holds one real fill for each of the 16 mapped values. `test_every_mapped_dir_has_a_real_captured_fill` requires the catalog and the mapping to be the same set; each catalog fill must agree with its mapping; Settlement and ADL have close/reduce/reject tests (including partial, wrong side, no position, overshoot or flip) |
| **F1-3** `dir` validated late | Kept deliberately, now with the reason and a guard. Rejecting unknown values in the adapter would fail the whole page and drop the wallet's entire history at load time because of one possibly later fill, which is the F3-1 look-ahead in a backtest. New architecture rule: only `domain/direction.py` reads `Fill.dir` (`.dir` or `getattr(..., "dir")`), so no code can act on an unvalidated value | 3 planted reads detected; `Fill(dir=...)`, `replace(fill, dir=...)` and `dir(module)` are not flagged |
| **F1-4** api-notes contradicted itself on liquidations | §3 now says: the liquidated user's own fill carries `Liquidated …`, the counterparty's keeps an ordinary `dir`. The old line was based on counterparty fills only. §4 has the forced-trade table and the validation rationale; §10's open items are struck through as observed. §3 also records the `closedPnl` rounding | — |
| **Mutation re-run after the F1 work** (2026-09-25) | The new forced-close check in `direction.py` had 7 survivors, 5 of them real: fractional positions (`end > 0` → `> 1`, `start > 0` → `> 1`), a zero-size ADL (`<` → `<=`), and a position of exactly 1 (two overlapping `start == 0` checks). Tests added for these; the check is simplified to `start != 0 and (end == 0 if settlement else 0 <= end / start < 1)` | Fresh mutmut run: **1742 mutants, 1722 killed (3 by timeout), 20 survive (98.9%)**, all re-verified on the full suite. They are exactly the 20 equivalent mutants listed below |

All audit findings are now closed or explained.
